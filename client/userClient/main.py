...
class ClientLogin:
    ...
    def Login(self):
        ...
        self.loginButton = tk.Button(self.loginFrame, text="Login", background="#d60000",
                                     ...
                                     command=lambda: ClientLogin.Forward(self, f'{usr.get()},{psw.get()}'))
    ...
    def Styling(self):
        ...
        self.activeGrowery = tk.Label(self.styleFrame, text=f'', background="#363636", foreground="#d9d9d9")
        self.activeGrowery.place(relx=0, rely=0.9, relheight=0.1, relwidth=1)
    ...
    def Forward(self, message):
        ...
        while not connected:
            try:
                ...
                self.activeGrowery['text'] = 'Connection to server not available'
                time.sleep(10)
                break
        try:
            UserClient.Send(client ,'UserClient')
...
class ClientMonitor:
    def __init__(self, client):
        ...
        ClientMonitor.Notification(self.root, client)
        ClientMonitor.Main(self.root, client)

        self.root.mainloop()

    def Notification(self, client):
